...
met <- read.csv(file=paste0(dropbox_dir, '/Data/NutrientExperiment/Oxygen18/InputTermsO18Metabolism_SSCN_Oct2018.csv'), 
                header=TRUE, sep=",", na.strings="NA", dec=".", strip.white=TRUE, stringsAsFactors = F)

met$lake.area.km2<-0.153
...
# met<-met[met$Date>as.Date("2018-09-01"),]
...
met.fun <- function(args){
  ...
  zmix <- met$zmix.m
  do <- met$ODO.mg.L
  do.pct.sat <- met$ODOsat..
  delo18.o2 <- met$d180_02.vs.VSMOW
